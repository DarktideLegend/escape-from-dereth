...
namespace ACE.Server.Entity
{
    ...
    public class Landblock : IActor
    {
        ...
        public bool AddWorldObject(WorldObject wo)
        {
            ...
            var result = ProcessAddedObject(wo, RealmRuleset);
            ...
            return AddWorldObjectInternal(result);
        }

        private WorldObject ProcessAddedObject(WorldObject wo, AppliedRuleset ruleset)
        {
        ...
        private WorldObject ProcessHideoutObject(WorldObject wo, AppliedRuleset ruleset)
        {
        ...
        private WorldObject ProcessHomeRealm(WorldObject wo, AppliedRuleset ruleset)
        {
            switch(wo.WeenieType)
            {
        ...
        private WorldObject ProcessHomeRealmPortal(Portal portal, AppliedRuleset ruleset)
        {
            if (portal.Name.Contains("Town Network")) // blacklist town network
            {
                return null;
            }
        ...
        private WorldObject ProcessHomeRealmCreature(Creature creature, AppliedRuleset ruleset)
        {
        ...
        private WorldObject ProcessHellgateObject(WorldObject wo, AppliedRuleset ruleset)
        {
            switch(wo.WeenieType)
            {
                case WeenieType.Door:
        ...
        private WorldObject ProcessHellgateCreature(Creature creature, AppliedRuleset ruleset)
        {
            var hellgate = HellgateManager.GetHellgate(creature.Location.Instance);

            if (hellgate != null)
            {
                var hellgateCreature = CreateForgottenMonster(hellgate.Tier);
                hellgateCreature.Location = new Position(creature.Location);
                CreatureRealmMutate(hellgateCreature, ruleset);
                MutateDeathTreasureTypeByTier(hellgateCreature);
                creature.Destroy();
                return hellgateCreature;
            }
        ...
        private void CreatureRealmMutate(Creature creature, AppliedRuleset ruleset)
        {
        ...
        private Creature CreateForgottenMonster(int tier)
        {
            if (tier == 1)
        ...
        private Creature CreatureHomeRealmMutate(Creature creature)
        {
            if (creature.IsGenerator || creature is Vendor || creature is Pet)
                return creature;
